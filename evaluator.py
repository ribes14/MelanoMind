...
def evaluate_submission(submission_file: str, ground_truth_file: str):
    ...
    merged_df = pd.merge(
        sub_df, gt_df, on="isic_id", how="inner", suffixes=("_pred", "_true")
    )

    # Ensure the columns exist
    if "target_pred" not in merged_df.columns or "target_true" not in merged_df.columns:
        raise ValueError(
            "Merged data does not have the expected 'target_pred' or 'target_true' columns."
        )
    ...
    try:
        ...
        auc = None
        print(
            "[WARNING] AUC could not be computed. Make sure y_true contains both 0 and 1 classes."
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate a submission CSV against ground truth."
    )
    parser.add_argument(
        "--submission", required=True, help="Path to submission CSV file."
    )
    parser.add_argument(
        "--ground_truth",
        required=True,
        help="Path to ground truth CSV file (professors-test-metadata.csv).",
    )
